...
libIMEToolsRule = do
  "libime-tools" ~> do
    ...
    cmd_ "cmake" "-B" (libIMESrc </> "build") "-G" "Ninja" 
      [
      ...
      libIMESrc
    cmd_ "cmake" "--build" (libIMESrc </> "build") "--target"
      [
    ...
    Exit c <- cmd "cmake" "--install" (libIMESrc </> "build") "--component" "lib"
    Exit c <- cmd "cmake" "--install" (libIMESrc </> "build") "--component" "header"
    Exit c <- cmd "cmake" "--install" (libIMESrc </> "build") "--component" "tools"
    Exit c <- cmd "cmake" "--install" (libIMESrc </> "build") "--component" "Devel"
    cmd_ "echo" ("installed to " <> outputDir)
...
lmRule = do
  ...
  outputDir </> "sc.lm" %> \out -> do
    ...
    cmd_ (AddEnv "LD_LIBRARY_PATH" (outputDir </> "lib")) (outputDir </> "bin" </> "libime_slm_build_binary") "-s -a 22 -q 8 trie" src out
  outputDir </> "sc.lm.predict" %> \out -> do
    let src1 = outputDir </> "sc.lm"
        src2 = outputDir </> "lm_sc.arpa"
    need ["libime-tools", src1, src2]
    cmd_ (AddEnv "LD_LIBRARY_PATH" (outputDir </> "lib")) (outputDir </> "bin" </> "libime_prediction") src1 src2 out
...
pinyinDictRule = do
  ...
  (outputDir </>) . (<.> "dict") <$> dictNames |%> \out -> do
    ...
    need ["libime-tools", src]
    cmd_ (AddEnv "LD_LIBRARY_PATH" (outputDir </> "lib")) (outputDir </> "bin" </> "libime_pinyindict") src out
...
tableDictRule = do
  ...
  (outputDir </>) . (<.> "main.dict") <$> tableDictNames |%> \out -> do
    ...
    need ["libime-tools", src]
    cmd_ (AddEnv "LD_LIBRARY_PATH" (outputDir </> "lib")) (outputDir </> "bin" </> "libime_tabledict") src out
...
pinyinExtraDictRule = do
  let dict name =
        outputDir </> name <.> "dict" %> \out -> do
          ...
          cmd_ (AddEnv "LD_LIBRARY_PATH" (outputDir </> "lib")) (outputDir </> "bin" </> "libime_pinyindict") txt out
  dict "chaizi"
  dict "emoji"
